...
def EncodePayload(PayloadName):
    ...
    if os.path.isfile("./payloads/" + PayloadName) == False:
        ...
        return
    print (chr(10) + "*** Your choice is " + green(PayloadName) + chr(10) + "*** Your encoded Payload will be created as " + green(PayloadName[:len(PayloadName) - 4] + "_inject.bin"))
...
def RenamePayload():
    ...
    if re.match('^[a-zA-Z_]+.txt', Payloads[ChosenPayload-1]) == None:
        RENAMEDPayload += Payloads[ChosenPayload-1][x]
    RENAMEDPayload += "_RENAMED"
